# 一：项目简介
基于ROB（重排序缓冲器）扩展Tomasulo算法是一种用于动态调度指令执行的技术，常用于乱序执行的超标量处理器中。该算法通过指令流出、执行、写结果、确认等步骤实现指令的乱序执行和并行处理。从而提高指令级并行度和处理器的吞吐量。<br>
# 二、项目原理
## 前置知识
### Tomasulo算法：
优点：<br>
...
## 程序执行的结果
![image](https://github.com/xxwang0/ROB/assets/147365280/88f3c020-12ba-4b89-915c-a999553b34ec)

# 四、技术路线/技术难点
## 技术路线：
1. 指令流出逻辑的实现：<br>
   * 首先需要理解和实现指令流出的过程。从寄存器文件、ROB 和保留站（RS）中分配和释放资源。<br>
   * 确保正确处理指令的就绪状态，例如通过检查  Ready 标志来判断指令是否可以执行。<br>
2. 状态管理：<br>
   * 实现寄存器状态表（RegStat）和 ROB 和保留站，用于跟踪每个寄存器和 ROB 项的状态。确保能够正确地更新和查询这些状态，以便指导指令的流出和执行。<br>
3. 指令执行：<br>
   * 整数运算、浮点运算、加载和存储操作。处理数据依赖关系。<br>
4. 数据相关性检测：<br>
   * 维护数据相关性，以确保指令在满足数据依赖关系的条件下执行。这可能涉及到检查和更新保留站中操作数就绪的状态。<br>
5. 内存和寄存器模拟：<br>
...
3. 性能优化：<br>
   * 通过并行处理、缓存优化和指令调度等技术手段来提高仿真器的运行效率。<br>
4. 调试和测试：<br>
